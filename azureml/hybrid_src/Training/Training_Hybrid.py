...
def train_hybrid(args):
    ...
    from Model.CSIHybrid import CSIHybrid

    checkpoint_path = args.checkpoint_path or os.path.join(path_cfg.checkpoints_dir, "hybrid_best.pt")
    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 80)
    print("Hybrid Training Run")
    print("=" * 80)
    ...
    model = CSIHybrid(num_classes=7).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the hybrid CSI model")
    ...
    parser.add_argument("--learning_rate", type=float, default=5e-4, help="AdamW learning rate")
